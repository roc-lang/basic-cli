...
fn tcp_heap() -> &'static Mutex<RefCell<RefcountedResourceHeap<BufReader<TcpStream>>>> {
...
pub unsafe extern "C" fn roc_dealloc(c_ptr: *mut c_void, _alignment: u32) {
    ...
    {
        let mut guard = tcp_heap().lock().unwrap();
        let heap = guard.get_mut();
        if heap.in_range(c_ptr) {
            heap.dealloc(c_ptr);
            return;
...
#[no_mangle]
pub extern "C" fn roc_fx_fileReadLine(file: RocBox<()>) -> RocResult<RocList<u8>, RocStr> {
    let buf_reader: &mut BufReader<File> = RefcountedResourceHeap::box_to_resource(file);
...
pub extern "C" fn roc_fx_tcpReadUpTo(
    ...
    let stream: &mut BufReader<TcpStream> = RefcountedResourceHeap::box_to_resource(stream);

    let mut chunk = stream.take(bytes_to_read);

    match chunk.fill_buf() {
        Ok(received) => {
            // TODO: write our own duplicate of `to_vec` that directly fills a `RocList<u8>`.
            // This adds an extra O(n) copy.
...
pub extern "C" fn roc_fx_tcpReadUntil(
    ...
    // TODO: write our own duplicate of `read_until` that directly fills a `RocList<u8>`.
    // This adds an extra O(n) copy.
    let mut buffer = vec![];
