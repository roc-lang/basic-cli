...
main! = |_args| {
	...
	stream : Tcp.Stream
	stream = Tcp.connect_with_timeout!("127.0.0.1", 8085, 1_000) ? |err| ConnectFailed(err)
...
verify_stream_methods! = |stream| {
	stream.write_with_timeout!([1, 2, 3], 1_000)?
	exact_bytes = stream.read_exactly_with_timeout!(3, 1_000)?
	expect exact_bytes == [1, 2, 3]

	stream.write_utf8_with_timeout!("until|", 1_000)?
	until_bytes = stream.read_until_with_timeout!(124, 64, 1_000)?
	expect until_bytes == [117, 110, 116, 105, 108, 124]

	stream.write_with_timeout!([42], 1_000)?
	up_to_bytes = stream.read_up_to_with_timeout!(1, 1_000)?
...
run! = |stream| {
	...
	match Stdin.line!() {
		...
		Ok(out_msg) => {
			stream.write_utf8_with_timeout!("${out_msg}\n", 5_000) ? |err| TcpWriteFailed(err)
			in_msg = stream.read_line_with_timeout!(1_048_576, 5_000) ? |err| TcpReadFailed(err)
			Stdout.line!("< ${in_msg}")?
			run!(stream)
		}
	}
