...
impl RocFunction_84 {
    pub fn force_thunk(mut self, arg0: ()) -> roc_std::RocResult<roc_std::RocStr, i32> {
        extern "C" {
            fn roc__mainForHost_0_caller(
                arg0: *const (),
                closure_data: *mut u8,
                output: *mut roc_std::RocResult<roc_std::RocStr, i32>,
            );
        }
...
fn mainForHost(arg0: &roc_std::RocStr) -> RocFunction_84 {
    extern "C" {
        // #[link_name = "roc__mainForHost_1_exposed_generic"]
        // fn roc_main(output: *mut u8, arg: &RocStr);
        fn roc__mainForHost_1_exposed_generic(_: *mut RocFunction_84, _: &roc_std::RocStr);
    }
...
pub extern "C" fn rust_main() {
    let arg = RocStr::from("this is the arg");

    let func = mainForHost(&arg);
    let result = func.force_thunk(());

    dbg!(&result);

    let _ = arg; // Extend the lifetime of arg so it doesn't get deallocated too soon
}
...
pub extern "C" fn roc_fx_dirList(
    ...
    // // Current problem: this doesn't reproduce, but the more complex one below does.
    ...
    //     "3abcdefghij",
    //     "asdfadfadfaDSF",
    //     "asdfafdgdfkgjsjfgjanfdgjksfg",
    // ];
    // let iter = vec.iter().map(|str| RocList::from_slice(str.as_bytes()));

    // // let answer: RocList<RocList<u8>> = RocList::from_iter(Vec::from_iter(iter).into_iter());
    // let answer: RocList<RocList<u8>> = RocList::from_iter(iter);

    // panic!("GOT ANSWER! {}", answer.len());

    let dir = std::env::current_dir().unwrap().join("temp");
    ...
    .iter()
    .for_each(|str| {
        let file_path = std::path::PathBuf::from(&dir).join(str);
        ...
        .map(|opt_dir_entry| {
            match opt_dir_entry {
                Ok(entry) => {
                    ...
                    os_str_to_roc_path(entry.path().into_os_string().as_os_str())
                    // RocList::from_slice(entry.as_bytes())
                }
                Err(_) => {
    ...
    match std::fs::read_dir("tests") {
        Ok(dir_entries) => {
            // let mut answer = RocList::default();

            // for entry in dir_entries {
            //     match entry {
            //         Ok(entry) => {
            //             answer.push(os_str_to_roc_path(
            //                 entry.path().into_os_string().as_os_str(),
            //             ));
            //         }
            //         Err(_) => {
            //             todo!("handle dir_entry path didn't resolve")
            ...
            // let iter = dir_entries.map(|opt_dir_entry| match opt_dir_entry {
            ...
            // let vec: Vec<Result<&'static str, ()>> = vec![
            //     Ok("tests/Dockerfile"),
            //     Ok("tests/node_modules"),
            //     Ok("tests/build.js"),
            //     Ok("tests/json"),
            //     Ok("tests/.gitignore"),
            //     Ok("tests/package-lock.json"),
            //     Ok("tests/package.json"),
            // ];

            // let iter = vec.into_iter().map(|opt_dir_entry| match opt_dir_entry {
            //     Ok(entry) => {
            //         println!("Processing entry: {:?}", entry);
            //         // os_str_to_roc_path(entry.path().into_os_string().as_os_str())
            //         RocList::from_slice(entry.as_bytes())
...
fn os_str_to_roc_path(os_str: &OsStr) -> RocList<u8> {
    ...
    println!(
        "making roclist from this many bytes: {}",
        os_str.as_bytes().len()
    );

    let list = RocList::from(os_str.as_bytes());
    println!("made it!");
    list
}
