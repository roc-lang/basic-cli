...
## after the `?`, if it has one, but before any `#` it might have. Passing `""` removes the `?` (if there was one).
## ```
...
## the `?`, if it has one, but before any `#` it might have. Returns `""` if the URL has no query.
## ```
